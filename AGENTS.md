...
The module path is `github.com/hackafterdark/phosphor`.
...
internal/
  ...
  config/
    ...
    load.go                        phosphor.json loading and validation
  ...
  hooks/                           Hook engine: runs user shell commands on hook events
    ...
    input.go                       Stdin payload builder, env vars, stdout parsing (Phosphor + Claude Code compat)
...
- **Pub/sub**: `internal/pubsub` for decoupled communication between agent,
  UI, and services.
- **Hooks**: User-defined shell commands in `phosphor.json` that fire before
