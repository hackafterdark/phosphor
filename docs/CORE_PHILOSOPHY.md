...
Phosphor is not a general-purpose chat interface. It is a coding kernel that integrates deep structural intelligence (AST), filesystem sandboxing, and local observability directly into the agent's execution loop.
...
An Agentic Laboratory: We value a lean, stable core over feature bloat. Phosphor is built to be a workbench; we encourage you to use it as a laboratory to experiment, learn, and prototype your own agentic ideas. Whether you want to borrow a component, test a new tool, or fork the runtime to build a highly specialized assistant, we provide the kernel—you build the machine. We believe the most powerful agents are those tailored by the developer, and we're here to provide the solid foundation for you to make it your own.

## 2. Security (Hardened by Design)

Phosphor treats the workspace as a strict, high-integrity sandbox. We employ a Defense-in-Depth model:
...
AST-Aware Editing: The edit and multiedit tools leverage Tree-sitter for real-time AST parsing. We validate syntax and check for missing elements before writing to disk, ensuring the build is never broken by hallucinations.
