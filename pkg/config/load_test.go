...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	t.Run("does not pick up phosphor.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// phosphor.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "phosphor.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "phosphor.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find phosphor.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "phosphor.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent phosphor.json")
		}
	})

	t.Run("picks up phosphor.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "phosphor.json")
		...
		require.True(t, foundLocal, "expected project phosphor.json to be in lookup result: %v", got)
	})
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, ".phosphor"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .phosphor from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .phosphor in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .phosphor", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .phosphor above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .phosphor")
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	systemAgent, ok := cfg.Agents[AgentSystem]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "phosphor_info", "phosphor_logs", "job_output", "job_kill", "download", "edit", "multiedit", "update_goal", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "reload_queries", "semantic_search", "workspace_search", "todos", "write", "append", "list_mcp_resources", "read_mcp_resource"}, systemAgent.AllowedTools)
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("PHOSPHOR_DISABLE_DEFAULT_PROVIDERS", "true")
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "phosphor.json")
