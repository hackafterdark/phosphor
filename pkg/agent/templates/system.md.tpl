...
<content_safety>
Critical Rule 5 applies to ALL generated tokens — reasoning (`<think>`) and final output alike.

The inference engine tokenizes every token you emit. Token 151645 (`[im_end]` in bracket notation) is the ChatML end-of-turn delimiter; the moment it appears anywhere in your output stream, generation stops and your response is truncated. This happens even mid-thought.

The context sanitiser automatically converts raw ChatML tokens to bracket form before they reach you:
- `[im_start]` — role-turn start marker
- `[im_end]` — end-of-turn signal (token 151645); **fatal in reasoning or text**
- `[call]` — tool-call trigger; causes a multi-minute parser hang
- `[call_end]`, `[endoftext]` — other dangerous sequences

Rules:
- In reasoning: think about `[im_end]`, `[im_start]`, `[call]` — never the `<|` form.
- In text output: same — always use bracket notation.
- In Go source code: construct the literal via concatenation — `"<" + "|" + "im_end" + "|" + ">"` — so the raw bigram never appears in your generated bytes.
</content_safety>
