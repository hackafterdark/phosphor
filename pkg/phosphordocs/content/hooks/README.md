...
the agent lifecycle, allowing you to both build on top of Phosphor, customize
...
- Phosphor ships with a builtin `phosphor-hook` skill write, edit, and configure
  hooks; just tell Phosphor how to configure Phosphor
- Phosphor currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `phosphor.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/phosphor/phosphor.json`), use an absolute path instead.
...
# ($PHOSPHOR_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$PHOSPHOR_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Phosphor's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Phosphor falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `PHOSPHOR=1`, `AGENT=phosphor`, and
  `AI_AGENT=phosphor` on top of the `PHOSPHOR_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Phosphor cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Phosphor abandons it, logs a
...
Hooks can be added to your `phosphor.json` (or `.phosphor.json`) at both the global
...
> work fine in project-level `phosphor.json` because the project root is also
> your working directory. For **global** config (`~/.config/phosphor/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/phosphor/phosphor.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/phosphor/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Phosphor's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Phosphor treats that as affirmative pre-approval and
...
| ---------------------------- | ---------------------------------------------- |
| `PHOSPHOR`                      | Always `1` when running under Phosphor.           |
| `AGENT`                      | Always `phosphor`.                                |
| `AI_AGENT`                   | Always `phosphor`.                                |
| `PHOSPHOR_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `PHOSPHOR_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `PHOSPHOR_SESSION_ID`           | Current session ID.                            |
| `PHOSPHOR_CWD`                  | Working directory.                             |
| `PHOSPHOR_PROJECT_DIR`          | Project root directory.                        |
| `PHOSPHOR_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `PHOSPHOR_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `PHOSPHOR`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Phosphor?" the same way in
either context.
...
Hooks communicate back to Phosphor via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `phosphor.json`.
...
If a hook exceeds its timeout, Phosphor cancels its context and treats the
...
if echo "$PHOSPHOR_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Phosphor to pre-approve the call:
...
case "$PHOSPHOR_TOOL_INPUT_COMMAND" in
...
if [[ "$PHOSPHOR_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $PHOSPHOR_TOOL_NAME\" >> ./tools.log" }
```
...
Phosphor hooks are broadly compatible with [Claude Code
...
hooks run under Phosphor unchanged. This document covers the Phosphor-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Phosphor treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Phosphor pre-approves the tool call and skips the permission
