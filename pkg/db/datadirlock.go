...
// already in use by another phosphor process.
var ErrDataDirLocked = errors.New("data directory already in use by another phosphor process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to phosphor.db so users can `ls` and find it.
const dataDirLockFile = "phosphor.lock"
...
// {dataDir}/phosphor.lock. If the lock is already held by another
...
// PHOSPHOR_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("PHOSPHOR_SKIP_DATADIR_LOCK"))
	return v
}
