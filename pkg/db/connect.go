...
// phosphor process from racing the storage.
...
// workspace-bootstrap path opts in. PHOSPHOR_SKIP_DATADIR_LOCK still
...
// WithSkipMigrations configures Connect to skip running database migrations.
func WithSkipMigrations(skip bool) ConnectOption {
	return func(o *connectOptions) { o.skipMigrations = skip }
}

// WithDatabaseName configures a custom database file name (defaults to "phosphor.db").
func WithDatabaseName(name string) ConnectOption {
	return func(o *connectOptions) { o.databaseName = name }
}
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	// phosphor process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second phosphor against the
...
func Release(dataDir string, opts ...ConnectOption) error {
	var cfg connectOptions
	for _, opt := range opts {
		opt(&cfg)
	}
...
// Migrate runs database migrations on the given connection.
