...
phosphor run "Guess my 5 favorite Pokémon"

# Run a non-interactively with pipes and redirection
cat README.md | phosphor run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
phosphor --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
phosphor --yolo

# Run with custom data directory
phosphor --data-dir /path/to/custom/.phosphor

# Continue a previous session
phosphor --session {session-id}
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("Phosphor crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/hackafterdark/phosphor/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
// enabled via the PHOSPHOR_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("PHOSPHOR_CLIENT_SERVER"))
	return v
}
...
func ensureServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	// server log file. phosphorlog.Setup uses sync.Once internally, so the
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize phosphor server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/phosphor/server-<safeHost>/start.lock.
//
...
// Overridable via PHOSPHOR_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("PHOSPHOR_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start phosphor server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach phosphor server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("PHOSPHOR_DISABLE_METRICS")); v {
		return false
	}
...
func createDotPhosphorDir(dir string) error {
