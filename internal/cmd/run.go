...
curl https://charm.land | phosphor run "Summarize this website"

# Read from a file
phosphor run "What is this code doing?" <<< prrr.go

# Redirect output to a file
phosphor run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
phosphor run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
phosphor run --verbose "Generate a README for this project"

# Continue a previous session
phosphor run --session {session-id} "Follow up on your last response"

# Continue the most recent session
phosphor run --continue "Follow up on your last response"
...
func runNonInteractive(
	...
	if !hideSpinner && stderrTTY {
		var themeOpt string
		if ws.Config != nil && ws.Config.Options != nil && ws.Config.Options.TUI != nil {
			themeOpt = ws.Config.Options.TUI.Theme
		}
		t := styles.Theme(themeOpt, ws.Config.Models[config.SelectedModelTypeLarge].Provider, ws.Path)
...
// streaming SSE events into a final, complete stdout for `phosphor run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `phosphor run` exited
...
// prefix returned by phosphor session list.
