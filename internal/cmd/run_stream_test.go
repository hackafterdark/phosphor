...
// with reason=tool_use and used to terminate `phosphor run` early via
...
// subscribers (e.g. a TUI watching workspace events while `phosphor
...
// non-nil error from `phosphor run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `phosphor run` waits) exits cleanly rather
...
// queue scenario: `phosphor run --continue` attaches to a session
