...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["PHOSPHOR_EVENT"])
	require.Equal(t, "bash", envMap["PHOSPHOR_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["PHOSPHOR_SESSION_ID"])
	require.Equal(t, "/work", envMap["PHOSPHOR_CWD"])
	require.Equal(t, "/project", envMap["PHOSPHOR_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["PHOSPHOR_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["PHOSPHOR_TOOL_INPUT_FILE_PATH"])

	// Shared Phosphor markers must be present so hook-authored scripts can
	// detect they're running under Phosphor the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["PHOSPHOR"])
	require.Equal(t, "phosphor", envMap["AGENT"])
	require.Equal(t, "phosphor", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$PHOSPHOR_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("phosphor format still works", func(t *testing.T) {
		t.Parallel()
