...
// event into a closure nobody reads, and subscribers (`phosphor run`)
...
// publish a RunComplete for its RunID, hanging a `phosphor run` caller that
...
// than internal bookkeeping, which is the contract a `phosphor run` caller
...
// bookkeeping slice, not the published event a `phosphor run` caller awaits.
