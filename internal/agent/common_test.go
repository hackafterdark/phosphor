...
func hyperBuilder(model string) builderFunc {
	return func(t *testing.T, r *vcr.Recorder) (fantasy.LanguageModel, error) {
		provider, err := openaicompat.New(
			...
			openaicompat.WithAPIKey(os.Getenv("PHOSPHOR_HYPER_API_KEY")),
...
func testEnv(t *testing.T) fakeEnv {
	workingDir := filepath.Join("/tmp/phosphor-test/", t.Name())
...
func coderAgent(r *vcr.Recorder, env fakeEnv, large, small fantasy.LanguageModel) (SessionAgent, error) {
	...
	// independently of user config on `$HOME/.config/phosphor/phosphor.json`.
	...
	cfg.Config().Options.DisabledSkills = []string{"phosphor-config"}
	...
	allTools := []fantasy.AgentTool{
		tools.NewBashTool(env.permissions, env.workingDir, cfg.Config().Tools.Bash, cfg.Config().Options.Attribution, modelName),
		...
		tools.NewStructuralSearchTool(env.workingDir, nil),
