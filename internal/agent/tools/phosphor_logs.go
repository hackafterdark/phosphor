...
const PhosphorLogsToolName = "phosphor_logs"

//go:embed phosphor_logs.md.tpl
var phosphorLogsDescriptionTmpl []byte

var phosphorLogsDescriptionTpl = template.Must(
	template.New("phosphorLogsDescription").
		Parse(string(phosphorLogsDescriptionTmpl)),
)

type phosphorLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func phosphorLogsDescription() string {
	return renderTemplate(phosphorLogsDescriptionTpl, phosphorLogsDescriptionData{
...
type PhosphorLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewPhosphorLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		PhosphorLogsToolName,
		phosphorLogsDescription(),
		func(ctx context.Context, params PhosphorLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			ctx, span := otel.StartSpan(ctx, "execute_tool phosphor_logs")
			defer span.End()
			span.SetAttributes(
				attribute.String("gen_ai.tool.name", PhosphorLogsToolName),
				attribute.String("gen_ai.tool.call.id", call.ID),
				attribute.String("gen_ai.tool.call.arguments", call.Input),
			)
			result := runPhosphorLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runPhosphorLogs reads and formats the last N log entries from the given file.
func runPhosphorLogs(logFile string, params PhosphorLogsParams) string {
