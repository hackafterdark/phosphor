...
const PhosphorInfoToolName = "phosphor_info"

//go:embed phosphor_info.md
var phosphorInfoDescription string

type PhosphorInfoParams struct{}

func NewPhosphorInfoTool(
	...
	return fantasy.NewAgentTool(
		PhosphorInfoToolName,
		phosphorInfoDescription,
		func(ctx context.Context, _ PhosphorInfoParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			ctx, span := otel.StartSpan(ctx, "execute_tool phosphor_info")
			defer span.End()
			span.SetAttributes(
				attribute.String("gen_ai.tool.name", PhosphorInfoToolName),
				attribute.String("gen_ai.tool.call.id", call.ID),
				attribute.String("gen_ai.tool.call.arguments", call.Input),
			)
			return fantasy.NewTextResponse(buildPhosphorInfo(cfg, lspManager, allSkills, activeSkills, skillTracker)), nil
		},
	)
}

func buildPhosphorInfo(cfg *config.ConfigStore, lspManager *lsp.Manager, allSkills []*skills.Skill, activeSkills []*skills.Skill, skillTracker *skills.Tracker) string {
