...
func TestPhosphorIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .phosphorignore file that ignores .log files
	require.NoError(t, os.WriteFile(".phosphorignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .phosphorignore file
	phosphorignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".phosphorignore"), []byte(phosphorignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .phosphorignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .phosphorignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .phosphorignore in subdir that ignores normal_nested
	subPhosphorignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".phosphorignore"), []byte(subPhosphorignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .phosphorignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .phosphorignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .phosphorignore")
