...
import (
	...
	"github.com/hackafterdark/phosphor/internal/ui/notification"
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case app.UpdateAvailableMsg:
		text := fmt.Sprintf("Phosphor update available: v%s → v%s.", msg.CurrentVersion, msg.LatestVersion)
		if msg.IsDevelopment {
			text = fmt.Sprintf("This is a development version of Phosphor. The latest version is v%s.", msg.LatestVersion)
		}
		ttl := 10 * time.Second
...
func (m *UI) handleSelectModel(msg dialog.ActionSelectModel) tea.Cmd {
	...
	if err := m.com.Workspace.UpdatePreferredModel(config.ScopeGlobal, msg.ModelType, msg.Model); err != nil {
		...
		if msg.ModelType == config.SelectedModelTypeLarge {
			...
			var themeOpt string
			if m.com.Config() != nil && m.com.Config().Options != nil && m.com.Config().Options.TUI != nil {
				themeOpt = m.com.Config().Options.TUI.Theme
			}
			m.applyTheme(styles.Theme(themeOpt, providerID, m.com.Workspace.WorkingDir()))
		}
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	if os.Getenv("PHOSPHOR_UI_DEBUG") == "true" {
...
func (m *UI) View() tea.View {
	...
	v.WindowTitle = "phosphor " + home.Short(m.com.Workspace.WorkingDir())
