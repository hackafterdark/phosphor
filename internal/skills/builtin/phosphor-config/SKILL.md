---
name: phosphor-config
description: Use when the user needs help configuring Phosphor — working with phosphor.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Phosphor behavior.
---

# Phosphor Configuration

Phosphor uses JSON configuration files with the following priority (highest to lowest):

1. `.phosphor.json` (project-local, hidden)
2. `phosphor.json` (project-local)
3. `$XDG_CONFIG_HOME/phosphor/phosphor.json` or `$HOME/.config/phosphor/phosphor.json` (global)
...
Phosphor runs selected string fields through an embedded bash-compatible
...
`phosphor.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Phosphor in a directory whose `phosphor.json` you haven't
reviewed.
...
> `.agents/skills`, `.phosphor/skills`, `.claude/skills`, `.cursor/skills`

Other options: `context_paths`, `progress`, `disable_notifications`, `disable_auto_summarize`, `disable_provider_auto_update`, `disable_default_providers`, `data_directory`, `initialize_as`.
    ...
    "PreToolUse": [
      {
        ...
        "command": ".phosphor/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".phosphor/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `PHOSPHOR_EVENT` | Event name (e.g. `PreToolUse`) |
| `PHOSPHOR_TOOL_NAME` | Name of the tool being called |
| `PHOSPHOR_SESSION_ID` | Current session ID |
| `PHOSPHOR_CWD` | Current working directory |
| `PHOSPHOR_PROJECT_DIR` | Project root directory |
| `PHOSPHOR_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `PHOSPHOR_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Phosphor also supports the Claude Code hook output format:
...
- `PHOSPHOR_GLOBAL_CONFIG` - Override global config location
- `PHOSPHOR_GLOBAL_DATA` - Override data directory location
- `PHOSPHOR_SKILLS_DIR` - Override default skills directory
