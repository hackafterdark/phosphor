...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "phosphor://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/phosphor/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "phosphor-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "phosphor://skills/phosphor-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "phosphor://skills/phosphor-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "phosphor-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "phosphor://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "phosphor://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "phosphor-hooks" {
			foundHooks = true
			require.Equal(t, "phosphor://skills/phosphor-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "phosphor://skills/phosphor-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "phosphor-config", Path: "phosphor://skills/phosphor-config"}, {Name: "phosphor-config", Path: "/user/phosphor-config"}},
			wantLen:  1,
			wantName: "phosphor-config",
			wantPath: "/user/phosphor-config",
		},
		{
			name:    "empty",
			input:   nil,
