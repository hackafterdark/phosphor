...
func PushPopPhosphorEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "PHOSPHOR_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "PHOSPHOR_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("PHOSPHOR_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if len(c.Options.GlobalContextPaths) == 0 {
		phosphorConfigDir := filepath.Dir(GlobalConfig())
		c.Options.GlobalContextPaths = []string{
			filepath.Join(phosphorConfigDir, "PHOSPHOR.md"),
			filepath.Join(filepath.Dir(phosphorConfigDir), "AGENTS.md"),
		}
	}
	...
	if str, ok := os.LookupEnv("PHOSPHOR_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("PHOSPHOR_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated phosphor.json placed above the project is never picked
...
func GlobalConfig() string {
	if phosphorGlobal := os.Getenv("PHOSPHOR_GLOBAL_CONFIG"); phosphorGlobal != "" {
		return filepath.Join(phosphorGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if phosphorCache := os.Getenv("PHOSPHOR_CACHE_DIR"); phosphorCache != "" {
		return phosphorCache
	}
...
func GlobalConfigData() string {
	if phosphorData := os.Getenv("PHOSPHOR_GLOBAL_DATA"); phosphorData != "" {
		return filepath.Join(phosphorData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/phosphor/`
	// for linux and macOS, it should be in `$HOME/.local/share/phosphor/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps Phosphor from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if phosphorSkills := os.Getenv("PHOSPHOR_SKILLS_DIR"); phosphorSkills != "" {
		return []string{phosphorSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/phosphor`.
...
// ProjectSkillsDir returns the default project directories for which Phosphor
