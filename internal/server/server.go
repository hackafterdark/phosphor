...
// socketDir returns the directory used for the Phosphor Unix socket.
...
// returned by [socketDir] and is named phosphor-<uid>.sock, falling
// back to phosphor.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/phosphor-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "phosphor.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("phosphor-%s.sock", usr.Uid)
	}
...
// Server represents a Phosphor server bound to a specific address.
type Server struct {
...
func (s *Server) installHandler() {
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/sessions/{sid}/rename", c.handlePostWorkspaceSessionRename)
	mux.HandleFunc("POST /v1/workspaces/{id}/sessions/{sid}/stateless", c.handlePostWorkspaceSessionStateless)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/generate-title", c.handlePostWorkspaceAgentSessionGenerateTitle)
