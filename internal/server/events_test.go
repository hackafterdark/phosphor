...
// are what non-interactive clients (e.g. `phosphor run`) rely on to
...
// through the SSE envelope. Without these correlators, `phosphor run`
...
// "agent failed" (returns non-zero from `phosphor run`) from "agent
